...
Lookup tables of the form $Q(s, a)$ where each state s has an entry for all possible actions at that state a, do not scale to enormous state ranges or countably infinite state spaces. This is because there are too many actions/states to store in memory and it  would take too long to learn the value of each state individually. 
